...
void main() {
    ...
    // q ??
    vec2 q = vec2(0.);
    q.x = smoothstep(-0.154*sin(u_time*2.),1.632,fbm(st*10.));
    q.y = smoothstep(1.046*sin(u_time*2.),5.048,fbm(st*10.));

    // r ??
    vec2 r = vec2(0.);
    
    // animation en fonction du temps, avec une indication de direction (vec2)
    //r.x = (fbm( sin(st) + -0.226*u_time ));
    r.y = fbm( st + 0.072*q + vec2(0.080,-0.510)+ -0.434*u_time);
