...
void main() {
    ...
    // q ??
    vec2 q = vec2(0.);
    q.x = smoothstep(-0.154*sin(u_time),1.808,fbm(st*10.));
    q.y = smoothstep(0.670*sin(u_time),5.048,fbm(st*10.));
    ...
    color = mix(vec3(0.085,0.017,0.022),
                vec3(0.085,0.005,0.004),
                clamp((f*f)*3.632,0.0,1.0));

    // couche 2 qui dépend de q
    color = mix(color,
                vec3(0.495,0.113,0.137),
                clamp(length(q),0.0,1.0));

    // couche 3 qui dépend de r
    color = mix(color,
                vec3(1.000,0.660,0.351),
