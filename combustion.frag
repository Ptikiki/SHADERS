...
void main() {
    ...
    // q ??
    vec2 q = vec2(0.);
    q.x = smoothstep(-1.018*sin(u_time),1.608,fbm(st*10.));
    q.y = smoothstep(0.982*sin(u_time),5.048,fbm(st*10.));
    ...
    color = mix(vec3(0.240,0.134,0.041),
                vec3(0.635,0.491,0.215),
                clamp((f*f)*3.632,0.0,1.0));

    // couche 2 qui dépend de q
    color = mix(color,
                vec3(0.090,0.065,0.014),
                clamp(length(q),0.0,1.0));

    // couche 3 qui dépend de r
    color = mix(color,
                vec3(0.570,0.357,0.188),
